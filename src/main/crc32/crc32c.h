...
#ifndef JIMI_CRC32C_H
#define JIMI_CRC32C_H
...
#include <wmmintrin.h>  // For SSE 4.2: _mm_clmulepi64_si128()
#else
#include <x86intrin.h>  // For SSE 4.2
#endif // _MSC_VER
#endif // __SSE4_2__
...
static inline uint32_t crc32c_combine_crc_u32(size_t block_size, uint32_t crc0, uint32_t crc1, uint32_t crc2, const uint32_t * next2) {
...
static inline uint32_t crc32c_hw_triplet_loop_u32(const char * data, size_t length, uint32_t crc_init)
{
    ...
    assert(((size_t)src8 & (kAlignment - 1)) == 0);
